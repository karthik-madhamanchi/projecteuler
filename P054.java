...
public class P054 {
	public static void main(String[] args) {
		String sample = "8C TS KC 9H 4S 7D 2S 5D 3S AC\n"
		...
		Scanner sampleIn = new Scanner(sample);
		int player1WinCount = 0;
		while (sampleIn.hasNext()) {
			Card[] cards1 = readCards(sampleIn);
			Card[] cards2 = readCards(sampleIn);
			if (isPlayer1Win(cards1, cards2)) {
				player1WinCount++;
			}
		}
		System.err.println(player1WinCount);

		Scanner in = new Scanner(System.in);
		int T = in.nextInt();
		for (int tc = 0; tc < T; tc++) {
			Card[] cards1 = readCards(in);
			Card[] cards2 = readCards(in);
			System.out.println("Player "
					+ (isPlayer1Win(cards1, cards2) ? "1" : "2"));
		}
	}

	static Card[] readCards(Scanner in) {
		Card[] cards = new Card[5];
		for (int i = 0; i < cards.length; i++) {
			cards[i] = new Card(in.next());
		}
		return cards;
	}

	static boolean isPlayer1Win(Card[] cards1, Card[] cards2) {
		sort(cards1);
		sort(cards2);
...
class Rank {
	...
	int comareTo(Rank other) {
		return (this.rank != other.rank) ? this.rank - other.rank
				: (this.highestValue1 != other.highestValue1) ? this.highestValue1
						- other.highestValue1
						: this.highestValue2 - other.highestValue2;
	}
}